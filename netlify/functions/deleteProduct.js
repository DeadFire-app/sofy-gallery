/**
 * deleteProduct: elimina item(s) por id o por image URL
 * POST JSON: { id? , image? }
...
const clean = (s, max=1500) => String(s ?? "").trim().slice(0, max);
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (resGet.status === 404) return err("data.json not found", 404);
